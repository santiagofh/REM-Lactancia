...
df_a04_l = pd.read_csv("output/2026_A04_SECCION_L.csv")

# Filtrar consultas por profesional
df_profesionales = df_a04_l[df_a04_l['descripcion_prestacion'].str.contains("Consulta de Lactancia por profesional", na=False)].copy()
...
for col in columnas_interes:
    if col in df_profesionales.columns:
        df_profesionales[col] = pd.to_numeric(df_profesionales[col], errors='coerce').fillna(0)
...
# Exportar a Excel (datos 2026 preliminar)
with pd.ExcelWriter("output/2026_A04_SECCION_L_TABLAS.xlsx") as writer:
    for nombre_hoja, tabla in tablas_por_categoria.items():
        tabla.to_excel(writer, sheet_name=nombre_hoja[:31])

print("Archivo Excel exportado con éxito (2026 preliminar).")